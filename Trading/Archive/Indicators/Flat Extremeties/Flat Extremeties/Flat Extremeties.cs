...
namespace cAlgo
{
    ...
    public class FlatExtremeties : Indicator
    {
        ...
        [Parameter("Lines To Show", Group = "Other Settings", DefaultValue = 50, MinValue = 1)]
        public int LinesToShow { get; set; }
        [Parameter("Line Extension", Group = "Other Settings", DefaultValue = 50, MinValue = 1)]
        ...
        private int _lineCounter, _lastCountedIndex;
        ...
        public override void Calculate(int index)
        {
            ...
            if (Math.Abs(high - open) < _tolerance)
                DrawLines(index, lineStart, lineStop, high, _highEqualsOpenColor);

            if (Math.Abs(high - close) < _tolerance)
                DrawLines(index, lineStart, lineStop, high, _highEqualsCloseColor);

            if (Math.Abs(low - open) < _tolerance)
                DrawLines(index, lineStart, lineStop, low, _lowEqualsOpenColor);

            if (Math.Abs(low - close) < _tolerance)
                DrawLines(index, lineStart, lineStop, low, _lowEqualsCloseColor);
        }

        private void DrawLines(int index, DateTime lineStart, DateTime lineStop, double lineLevel, Color lineColor)
        {
            Chart.DrawTrendLine((_lineCounter % LinesToShow).ToString(), lineStart, lineLevel, lineStop, lineLevel, lineColor, LineThickness, LineStyle.Solid);

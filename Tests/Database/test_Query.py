...
def test_interpolation():
    ...
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, schema="public", table="users")
...
def test_missing_interpolation():
    ...
    with pytest.raises(KeyError):
        q(PostgresAPI._PARAMETER_TOKEN_)  # schema missing

def test_interpolation_and_named_same_key():
    q = QueryAPI("SELECT * FROM ::x::.t WHERE x = :x:")
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, x="public")
...
def test_positional():
    ...
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, 10, 20)
...
def test_named_compiles_to_positional():
    ...
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, id=7, status="ACTIVE")
...
def test_named_and_positional_ordering():
    ...
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, "SVAL", id=1, x=3)
...
def test_duplicate_named_parameter():
    ...
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, id=99)
...
def test_interpolation_and_named():
    ...
    query, configuration, parameters = q(PostgresAPI._PARAMETER_TOKEN_, schema="public", table="users", id=5)
...
def test_missing_positional():
    ...
    with pytest.raises(ValueError):
        q(PostgresAPI._PARAMETER_TOKEN_, 10)

def test_missing_named():
    q = QueryAPI("SELECT * FROM t WHERE id = :id:")
    with pytest.raises(KeyError):
        q(PostgresAPI._PARAMETER_TOKEN_)  # id missing

def test_numbered_placeholders():
    q = QueryAPI("SELECT * FROM t WHERE a = :?: AND b = :x: AND c = :?:")
    query, configuration, parameters = q(OracleAPI._PARAMETER_TOKEN_, "A", "C", x="B")

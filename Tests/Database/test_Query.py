...
def test_interpolation():
    ...
    query, parameters = q(PostgresAPI._PARAMETER_TOKEN_, schema="public", table="users")
    assert query == "SELECT * FROM public.users"
    assert parameters is None

def test_positional():
    q = QueryAPI("SELECT * FROM t WHERE a = :?: AND b = :?:")
    query, parameters = q(PostgresAPI._PARAMETER_TOKEN_, 10, 20)
...
def test_named_compiles_to_positional():
    ...
    query, parameters = q(PostgresAPI._PARAMETER_TOKEN_, id=7, status="ACTIVE")
...
def test_named_and_positional_ordering():
    ...
    query, parameters = q(PostgresAPI._PARAMETER_TOKEN_, "SVAL", id=1, x=3)
...
def test_duplicate_named_parameter():
    ...
    query, parameters = q(PostgresAPI._PARAMETER_TOKEN_, id=99)
...
def test_interpolation_and_named():
    ...
    query, parameters = q(PostgresAPI._PARAMETER_TOKEN_, schema="public", table="users", id=5)
...
def test_numbered_placeholders():
    ...
    query, parameters = q(OracleAPI._PARAMETER_TOKEN_, "A", "C", x="B")
